...
def ms_split_bottle_split(
    ...
    if not use_theta and segsites is None:
        sys.exit(
            "ms_split_bottle_split: if use_theta is False, must provide number of segregating sites s"
        )
    ...
    timesAgoMs = [x / (4.0 * N0) for x in timesAgo]
    T3 = timesAgoMs[0]
    T2 = timesAgoMs[1]
    T1 = timesAgoMs[2]
    sizesMs = [x / (1.0 * N0) for x in sizes]
    thetaMs = 4 * N0 * mu * L
    N3 = sizesMs[0]
    N2 = sizesMs[1]
    N1 = sizesMs[2]
    ...
    if use_theta:
        cmd = "ms {numTotal} {numReps} -t {thetaMs} -I 3 {numSamples} {numSamples} {numSamples} -ej {T1} 2 1 -eN {T1} {N2} -eN {T2} {N1} -ej {T3} 3 1 -eN {T3} {N1} -seeds {seeds[0]} {seeds[1]} {seeds[2]} > {msfile}".format(
            **locals()
        )
    else:
        cmd = "ms {numTotal} {numReps} -s {segsites} -I 3 {numSamples} {numSamples} {numSamples} -ej {T1} 2 1 -eN {T1} {N2} -eN {T2} {N1} -ej {T3} 3 1 -eN {T3} {N1} -seeds {seeds[0]} {seeds[1]} {seeds[2]} > {msfile}".format(
            **locals()
        )
    return cmd
...
def ms_bottle_split(
    ...
    if reductionFactors is not None:
        sizes = sizes * reductionFactors  # effective sizes that account for propFemales
    N0 = sizes[-1]  # N0 = Ncurrent
    timesAgo = [times[-2] + times[-1], times[-1], 0.0]
    timesAgoMs = [x / (4.0 * N0) for x in timesAgo]
    sizesMs = [x / (1.0 * N0) for x in sizes]
    thetaMs = 4 * N0 * mu * L
    ...
    if use_theta:
        cmd = "ms {numTotal} {numReps} -t {thetaMs} -I 2 {numSamples} {numSamples} -ej {timesAgoMs[1]} 1 2 -eN {timesAgoMs[1]} {sizesMs[1]} -eN {timesAgoMs[0]} {sizesMs[0]} -seeds {seeds[0]} {seeds[1]} {seeds[2]} > {msfile}".format(
            **locals()
        )  # TODO untested
    else:
        cmd = "ms {numTotal} {numReps} -s {segsites} -I 2 {numSamples} {numSamples} -ej {timesAgoMs[1]} 1 2 -eN {timesAgoMs[1]} {sizesMs[1]} -eN {timesAgoMs[0]} {sizesMs[0]} -seeds {seeds[0]} {seeds[1]} {seeds[2]} > {msfile}".format(
            **locals()
        )  # TODO untested
    return cmd


def ms_bottle_epoch_split(
    numSamples,
    numReps,
    mu,
    L,
    ...
    if use_theta:
        cmd = "ms {numTotal} {numReps} -t {thetaMs} -I 2 {numSamples} {numSamples} -ej {timesAgoMs[2]} 1 2 -eN {timesAgoMs[1]} {sizesMs[1]} -eN {timesAgoMs[0]} {sizesMs[0]} -seeds {seeds[0]} {seeds[1]} {seeds[2]} > {msfile}".format(
            **locals()
        )
    else:
        cmd = "ms {numTotal} {numReps} -s {segsites} -I 2 {numSamples} {numSamples} -ej {timesAgoMs[2]} 1 2 -eN {timesAgoMs[1]} {sizesMs[1]} -eN {timesAgoMs[0]} {sizesMs[0]} -seeds {seeds[0]} {seeds[1]} {seeds[2]} > {msfile}".format(
            **locals()
        )
    return cmd
...
def run_ms_simulation(
    ...
    if len(seeds) != 3:
        sys.exit(
            "run_ms_simulation: seeds must have length 3, got {}".format(len(seeds))
        )
    if chromType == "A":
        reductionFn = simb.fA
    elif chromType == "X":
    ...
    reductionFactors = array(
        [reductionFn(p) for p in propFemales]
    )  # from the coalescent
    ...
    ms_data = msfunc.data_from_ms_file(
        msfile, average=False, write_kimtree=True, outfile=ktfile, segsites=S
    )  # this works regardless of whether segsites was in explicit argument to ms via -s or theta was provided via -t
...
def simSexBiasedBottleneckTwoPop(
    numSamples,
    numReps,
    mu,
    L,
    ...
    timesAgoMs = [x / (4.0 * N0) for x in timesAgo]
    sizesMs = [x / (1.0 * N0) for x in effSizes]
    thetaMs = 4 * N0 * mu * L
    ...
    cmd = "ms {numTotal} {numReps} -t {thetaMs} -I 2 {numSamples} {numSamples} -ej {timesAgoMs[2]} 1 2 -eN {timesAgoMs[1]} {sizesMs[1]} -eN {timesAgoMs[0]} {sizesMs[0]} -seeds {seeds[0]} {seeds[1]} {seeds[2]}".format(
        **locals()
    )
    cmd += " > {}".format(msfile)
...
def fitThreeEpoch(
    ...
    try:
        ...
        if funcName == "three_epoch_X0":  # do not optimize because c = 0.75
            ...
            popt = lrt.getXparams(
                poptA, funcName
            )  # write popt, params in terms of chrX. poptA is auto terms followed by c's
            ...
            funcName == "three_epoch_X1" or funcName == "three_epoch_X2"
        ):  # optimize chrX parameters to fit c's
            if funcName == "three_epoch_X1":
                func = lrt.three_epoch_X1
                func_ex = dadi.Numerics.make_extrap_log_func(func)
                params = array(
                    [nuB, nuF, TB, TF, thetaA, 0.75]
                ...
                upper_bound = [
                    ...
                    1e6,
                    1.11,
                ]  # only last bound matters, is for c
                lower_bound = [1e-4, 1e-1, 1e-4, 1e-4, 1e2, 0.57]
                fixed_params = array(
                    [nuB, nuF, TB, TF, thetaA, None]
                )  # c is last and free
            else:
                func = lrt.three_epoch_X2
                func_ex = dadi.Numerics.make_extrap_log_func(func)
                params = array(
                    [nuB, nuF, TB, TF, thetaA, 0.75, 0.75]
                )  # starting point for opt
                upper_bound = [
                    1,
                    10e2,
                    ...
                    1.11,
                    1.11,
                ]  # only last two bounds matters, is for c
                lower_bound = [1e-4, 1e-1, 1e-4, 1e-4, 1e2, 0.57, 0.57]
                fixed_params = array(
                    [nuB, nuF, TB, TF, thetaA, None, None]
                )  # c1 and c2 are last and free
            ...
            model = func_ex(
                poptA, ns, pts_l
            )  # should be same as at end of optimization
            ll_opt = dadi.Inference.ll(model, data)
            popt = lrt.getXparams(
                poptA, funcName
            )  # write popt, params in terms of chrX. poptA is auto terms followed by c's
...
def run_sb(fsfileA, fsfileX, outfileA, modelfileA):
    ...
    """
    (poptA, ll_optA, thetaA) = lrt.fitThreeEpoch(
        fsfileA, outfileA, modelfileA, isCluster=False
    )
...
def run_kimtree(datfileA, datfileX, treef, outdir, do_run=True, kimtree_bin=None):
    ...
    # on the original cluster. Now resolves from (in order):
    ...
    if do_run:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                "kimtree failed with exit code {}".format(result.returncode)
            )
