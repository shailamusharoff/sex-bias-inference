"""
Modified the following fn to take two more params
def plot_2d_comp_Poisson(model, data, vmin=None, vmax=None,

                         
From dadi distribution:
...
def difference_absolute(model, data, mask=None):
    """
    written by shaila. TODO could add to a custom version of Inference
    Return the absolute difference, (model - data), of model and data.
...
def plot_1d_fs(fs, fig_num=None, outfile=None, main=None, dpi=200):
    """
...
# shaila modified to take an outfile name and main title
def plot_1d_comp_multinom(model, data, fig_num=None, residual='Anscombe',
                          plot_masked=False, fs_labels=('model', 'data'), outfile=None, main=None, dpi=200):
    """
    ...
    plot_1d_comp_Poisson(model, data, fig_num, residual,
                         plot_masked, fs_labels=fs_labels, outfile=outfile, main=main, dpi=dpi)

# shaila modified to take an outfile name and main title; made dots smaller with "ms=0.5"
# modified to plot absolute difference rather than residual
def plot_1d_comp_Poisson(model, data, fig_num=None, residual='Anscombe',
    ...
    residual: 'Anscombe' for Anscombe residuals, which are more normally
              distributed for Poisson sampling. 'linear' for the linear
              residuals, which can be less biased. 'difference' for absolute difference.
    plot_masked: Additionally plots (in open circles) results for points in the
                 model or data that were masked.
    """
    if fig_num is None:
    ...
    if len(fs_labels) != 2:
        raise ValueError('fs_labels must be of length 2.')
    ax = pylab.subplot(2,1,1)
    pylab.semilogy(masked_data, '-ob', ms=0.5, label=fs_labels[1])    # this is the second labels arg
    pylab.semilogy(masked_model, '-or', ms=0.5, label=fs_labels[0])

    if plot_masked:
        pylab.semilogy(masked_data.data, '--ob', ms=0.5, mfc='w', zorder=-100, label=fs_labels[1])
        pylab.semilogy(masked_model.data, '--or', ms=0.5, mfc='w', zorder=-100, label=fs_labels[0])
    ...
    elif residual == 'difference':   # shaila added
        resid = difference_absolute(masked_model, masked_data)
    else:
        raise ValueError("Unknown class of residual '%s'." % residual)
    pylab.plot(resid, '-og', ms=0.5)
    if plot_masked:
        pylab.plot(resid.data, '--og', ms=0.5, mfc='w', zorder=-100)
...
def plot_single_2d_sfs(sfs, vmin=None, vmax=None, ax=None,
                       pop_ids=None, extend='neither', colorbar=True, outfile=None, dpi=200):
    """
...
def plot_2d_comp_multinom(model, data, vmin=None, vmax=None,
                          ...
                          pop_ids=None, axis_titles=('data','model'), residual='Anscombe',
                          adjust=True, outfile=None, dpi=200):
    """
    ...
    plot_2d_comp_Poisson(model, data, vmin=vmin, vmax=vmax,
                         ...
                         pop_ids=pop_ids, axis_titles=axis_titles, residual=residual,
                         adjust=adjust, outfile=outfile, dpi=dpi)
...
def plot_2d_comp_Poisson(model, data, vmin=None, vmax=None,
                         ...
                         adjust=True, outfile=None, dpi=200):
    """
    Poisson comparison between 2d model and data.  Edited by shaila.
    ...
    if len(axis_titles) != 2:
        raise ValueError('axis_titles must be of length 2.')
...
def plot_3d_spectrum(fs, fignum=None, vmin=None, vmax=None, pop_ids=None, outfile=None, dpi=200):
    """
